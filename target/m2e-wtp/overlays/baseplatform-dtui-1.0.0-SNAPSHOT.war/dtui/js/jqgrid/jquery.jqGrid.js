...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			/* 升级到4.5.2 modify by xychun 2013-10-15
			...
			*/
			...
			//如果总记录数为0，设置当前页为0 add by xychun 2013-10-15 
			if (ts.p.records == 0){
				ts.p.page = 0;
			}
			...
			//modified by xychun 2013.07.17
			//如果root为_self，表示不分页，行数据为data
//			drows = $.jgrid.getAccessor(data,dReader.root);
			if (dReader.root === '_self')
				drows = data;
			else
				drows = $.jgrid.getAccessor(data,dReader.root);
		...
		$("th",thr).each(function ( j ) {
			...
			//修改默认值为false, modify by xychun 2013-09-22
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$console.log("table width:"+nwidth);
			...
			if(shrink===true) {
				...
				if(vc  === 0) { return; }
				$console.log("col width:"+initwidth);
				...
				//表格宽度大于所有可见列的宽度总和才需要拉伸
				var needShrink = nwidth > $t.p.tblwidth;
				$.each($t.p.colModel, function(i) {
					if(this.hidden === false && !this.fixed){
						cw = this.widthOrg;
						//增加判断，只有表格宽度大于所有可见列的宽度总和才拉伸 add by xychun 2013-11-05
						if (needShrink)
				...
				cr =0;
				//增加判断，需要拉伸的时候按jqgrid的原始方式处理，不需要拉伸的时候，只设置表格宽度。modify by xychun 2013-11-05
				if (needShrink){
					if (hs) {
